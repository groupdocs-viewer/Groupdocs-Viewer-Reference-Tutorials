...
I dagens snabbrörliga digitala värld är det viktigt att kunna **konvertera EML till HTML** snabbt och med rätt datum‑tid‑presentation för arkivering, supportportaler och juridisk efterlevnad. Denna handledning guidar dig genom att rendera e‑postmeddelanden till HTML samtidigt som du tillämpar ett **anpassat datum‑tid‑format** och en **tidszonsförskjutning** med GroupDocs.Viewer för Java. I slutet har du en återanvändbar lösning som håller tidsstämplar korrekta och läsbara, perfekt för alla **email till HTML Java** arbetsflöden.
...
- Hur du **anpassar datum‑tid‑formatet** för dina e‑postmeddelanden (custom datetime java)  
- Hur du **ställer in tidszonsförskjutningen** för korrekta tidsstämplar (timezone offset java)  
...
### Steg 1: Ställ in utmatningskatalog och filsökväg
```java
...
```
*Förklaring:* `Path.of()` skapar en referens till mappen där HTML‑filen sparas. `resolve()` lägger till filnamnet.
...
```
*Förklaring:* `forEmbeddedResources()` samlar bilder och andra resurser direkt i HTML‑utdata.

### Steg 4: Ställ in anpassat datum‑tid‑format *(custom datetime java)*
```java
options.getEmailOptions().setDateTimeFormat("MM d yyyy HH:mm tt zzz");
```
*Förklaring:* Detta mönster visar månad, dag, år, timme, minut, AM/PM‑markör och tidszonsförskjutningen (`zzz`).

### Steg 5: Ställ in tidszonsförskjutning *(timezone offset java)*
```java
...
### Hur du justerar e‑postens tidszon i Java
Om du behöver **justera e‑postens tidszon** utöver enkla förskjutningar — till exempel hantera sommartidsändringar — kan du hämta rätt `TimeZone`‑objekt från `java.util.TimeZone`‑API:t med region‑ID:n som `"Europe/Paris"` eller `"America/New_York"` och skicka det till `setTimeZoneOffset`. Detta säkerställer att e‑postens tidsstämplar alltid visar korrekt lokal tid.
...
## Felsökningstips
- **FileNotFoundException:** Dubbelkolla sökvägarna som används i `Viewer` och `Path.of()`.  
- **Felaktiga tidsstämplar:** Verifiera att `TimeZone`‑ID:n matchar din målregion.  
- **Saknade bilder:** Säkerställ att du använde `HtmlViewOptions.forEmbeddedResources()`; annars kan externa resurser uteslutas.  
...
## Slutsats
Du har nu en komplett, produktionsklar metod för att **konvertera EML till HTML** med ett anpassat datum‑tid‑format och tidszonsförskjutning med GroupDocs.Viewer för Java. Detta förbättrar läsbarheten, säkerställer korrekta tidsstämplar och integreras sömlöst i arkiverings‑ eller supportarbetsflöden.
...
A: Omge renderingslogiken med en loop och återanvänd samma `HtmlViewOptions`‑instans för varje fil.

**Q: Vad händer om jag behöver stöd för andra e‑postformat som MSG?**  
A: GroupDocs.Viewer stödjer även MSG, PST och andra e‑postbehållare — byt helt enkelt filändelsen i `Viewer`‑konstruktorn.

**Q: Behöver jag en separat licens för varje server?**  
A: Licensiering sker per distribution; konsultera GroupDocs licensguide för fler‑serverscenarier.
